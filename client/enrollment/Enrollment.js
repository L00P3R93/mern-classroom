...
import {read, complete, enrollmentStats} from './api-enrollment.js'
...
export default function Enrollment({match}){
    ...
    const selectDrawer = (index) => event => { setValues({...values, drawer:index}) }
    ...
    const markComplete = () => {
        if(!enrollment.lessonStatus[values.drawer].complete){
            ...
            if(count == lessonStatus.length){updatedData.courseCompleted = Date.now()}
